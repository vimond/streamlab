...
import { Box, Button, Checkbox, Flex, Grid, Icon, Input, List, PseudoBox, Text, FormLabel } from '@chakra-ui/core';
...
const inputStyle = { padding: '0.1rem', border: 'none', height: 'auto', lineHeight: 'normal', marginBottom: '0.2rem' };

const renderHeaderRow = ({ id, name, value }: Header) => (
  <React.Fragment key={id}>
    {/*
            // @ts-ignore */}
    <Input type="text" defaultValue={name} isReadonly style={inputStyle} />
    {/*
            // @ts-ignore */}
    <Input type="text" defaultValue={value} isReadonly style={inputStyle} />
  </React.Fragment>
);
...
const StreamResourceFields: React.FC<{
  ...
  <>
    ...
    {/*
              // @ts-ignore */}
    <Input type="url" defaultValue={resource.url} isReadonly style={inputStyle} />
    ...
    {resource.technology !== BaseTech.AUTO && (
      <>
        ...
        {/*
              // @ts-ignore */}
        <Input type="text" defaultValue={getLabel(resource.technology, techLabels)} isReadonly style={inputStyle} />
      </>
    )}
