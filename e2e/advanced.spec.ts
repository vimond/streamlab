...
test('DRM playback that fails.', async (t) => {
  await t
    ...
    .expect(info.hasMessageContaining('Player error: DRM playback is blocked'))
    .ok()
    .click(advanced.stopButton)
    .click(top.tabs.history)
    .click(history.latestEntry)
...
test('Setting player options', async (t) => {
  await t
    ...
    // .expect(Selector('*').withAttribute('aria-label', 'Information panel').find('li').withText('In Player options, the Shaka Player').exists)
    .ok()
    .expect(player.playbackMonitor.exists)
    .ok()
    .wait(2000)
