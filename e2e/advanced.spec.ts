...
test('Inputting advanced stream details.', async (t) => {
  await t
    ...
    .click(advanced.streamDetails.addStreamHeader)
    ...
    .notOk()
    .click(advanced.streamDetails.addStreamHeader)
    .typeText(advanced.streamDetails.headerName.nth(1), firstHeaderName, { paste: true })
    .typeText(advanced.streamDetails.headerValue.nth(1), firstHeaderValue, { paste: true })
    .click(advanced.playButton)
    .wait(5000)
    .expect(info.hasMessageContaining('Player error: networkError/manifestLoadError.'))
    .ok()
    .click(top.tabs.history)
    .click(history.latestEntry)
    .expect(history.selectedEntry.streamUrl.value)
    .eql(streamUrl)
    .expect(history.selectedEntry.streamHeaders.nth(0).value)
    .eql(secondHeaderName)
    .expect(history.selectedEntry.streamHeaders.nth(1).value)
    .eql(secondHeaderValue)
    .expect(history.selectedEntry.streamHeaders.nth(2).value)
    .eql(firstHeaderName)
    .expect(history.selectedEntry.streamHeaders.nth(3).value)
    .eql(firstHeaderValue)
    .click(top.tabs.sharing)
    .expect(sharing.shareUrlField.textContent)
    .contains(reproduceShareUrlField('name', firstHeaderName))
    .expect(sharing.shareUrlField.textContent)
    .contains(reproduceShareUrlField('value', firstHeaderValue))
    .expect(sharing.shareUrlField.textContent)
